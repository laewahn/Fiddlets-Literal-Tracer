...
exports.functionChainFromLine = function(line, context) {
	...
	if (firstElement.value === undefined) {
		firstElement.value = context[firstElement.name];
	};

	var calls = [new ObjectCall(firstElement.name, firstElement.value)];

	parsedLine.slice(1).forEach(function(fn) {
		calls.push(new FunctionCall(fn.name, fn.params.map(function(e){
