...
(function(){
  ...
  Tracer.prototype.trace = function(source) {
    ...
    traceBody(parsed.body, tracingResults, contextMapping);
  
    return new tr.TracingResults(tracingResults, contextMapping);  
  };
  
  function traceBody(body, tracingResults, contextMapping) {
    tracingResults.__scopes = [];
    tracingResults.__identifiers = [];
  
    body.forEach(function(line) {
      evaluateLine(line, tracingResults, contextMapping);
    });
      
    return tracingResults; 
  }
  
  function evaluateLine(line, tracingResults, contextMapping)
  {
    switch(line.type) {
        case "VariableDeclaration": 
          evaluateVariableDeclaration(line.declarations, tracingResults, contextMapping);         
          break;
        ...
        case "ExpressionStatement":
          evaluateExpressionStatement(line.expression, tracingResults, contextMapping);
          break;
        case "ReturnStatement" :
          evaluateExpressionStatement(line.argument, tracingResults, contextMapping);
          break;
        default:
          // throw new Error("Unsupported type: " + line.type + " in\n" + JSON.stringify(line, null, 2));
          evaluateExpressionStatement(line, tracingResults, contextMapping);
      }
  }
  
  function evaluateVariableDeclaration(declarations, tracingResults, contextMapping) {
  
    declarations.forEach(function(declaration) {
      var varName = declareVariable(declaration, tracingResults, contextMapping);
  ...
  function declareVariable(declaration, tracingResults, contextMapping) {
    var varName = declaration.id.name;
    tracingResults[varName] = null;
    if (declaration.loc !== undefined) {
      contextMapping[varName] = contextMapping[varName] || [];
      contextMapping[varName].push(declaration.loc);
    }
  ...
  function evaluateExpressionStatement(expression, tracingResults, contextMapping) {
    switch(expression.type) {
      ...
      case "CallExpression" :
        if (expression.callee.object !== undefined) {
          if (expression.callee.object.name !== undefined) {
            contextMapping[expression.callee.object.name].push(expression.loc);
          }
        }
        break;
      default:
